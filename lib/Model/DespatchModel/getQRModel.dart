...
class DesQRModel{
DesQRheader? desQRheader;
String? message;
  int? stcode;
  String? exception;
  DesQRModel({
    required this.desQRheader,
  ...
  factory DesQRModel.fromJson(Map<String,dynamic>json,int stcode){
   
   if(json !=null){
    return DesQRModel(
  desQRheader: DesQRheader.fromJson(json), 
  stcode: stcode, 
  message: "success", 
  exception: null
  );
  ...
  factory DesQRModel.issues(Map<String,dynamic> jsons, int stcode){
 return DesQRModel(
  desQRheader: null, 
  stcode: stcode, 
  ...
  factory DesQRModel.error(String? exception,int stcode){
   return DesQRModel(
  desQRheader: null, 
  stcode: stcode, 
  message: "", 
...
class DesQRheader{
List<DesQRheaderList>? itemlist;
DesQRheader({
required this.itemlist
});
factory DesQRheader.fromJson(Map<String,dynamic> jsons){
  if(jsons['data'] !=null){
    var list=json.decode(jsons['data']) as List;
    List<DesQRheaderList> datalist=list.map((data)=>DesQRheaderList.fromJson(data)).toList();
    return DesQRheader(
      itemlist: datalist
      );
  }else{
...
class DesQRheaderList{
 ...
 factory DesQRheaderList.fromJson(Map<String,dynamic> json){
  // log("hgg::"+json["DocEntry"].toString());
return DesQRheaderList(
  ItemDisposition:json['ItemDisposition']??0,
